...
model ReportSend {
  ...
  windowEnd      DateTime
  recipients     Json             @default("[]")   // string[] snapshot at send time
...
enum ReportSendStatus {
  SENT          // digest emailed
  SKIPPED_EMPTY // fired but window had zero runs; window left open
  FAILED        // aggregation ran but mail delivery threw
}
...
- Add back-relation `sends ReportSend[]` to `ReportConfig`.
- `recipients` is snapshotted per send (config recipients may change later).
- `preview` sends are throwaway (self-only) and are **not** logged.
- `SKIPPED_EMPTY` rows explain schedule fires that produced no mail, so history has no unexplained gaps.
- `lastSentAt` stays on `ReportConfig` as the window cursor (cheap lookup); `ReportSend` is the audit record, not the cursor.
...
4. **Zero runs → write a `ReportSend` (`SKIPPED_EMPTY`), return without sending; leave `lastSentAt` unchanged.**
5. Else build the digest via `run-stats`, send to each recipient, set `lastSentAt = now`, write a `ReportSend` (`SENT`, or `FAILED` with `error` if delivery throws). All rows snapshot the window, recipients, and summary counts.
...
- `POST /reports/:id/preview` — `reports_edit`; render + send to requesting user only (not logged)
- `GET  /reports/:id/sends` — `reports_read`; paged send history, newest first
...
- Send-history view per report (list from `GET /reports/:id/sends`: sentAt, status, window, run count, pass rate, recipient count).
...
- Per-recipient unsubscribe / delivery tracking (history is at the send level, not per-address).

Add any of these when a concrete need arises.

## Affected files

- Data model: `schema.prisma` (`ReportConfig`, `ReportSend` + enums), new migration, `seed.ts`.
